...
def areaNormalize(totalQ_PerChannel, chstart=0):
    ...
    Q_perArea = [totalQ / a for totalQ, a in zip(totalQ_PerChannel, calculateAreas(chstart, cm=True))]
    return Q_perArea

def calculateAreas(chstart = 0,cm=True):
    ...
    if cm:
        ring_areas=[ring_area/100 for ring_area in ring_areas]
    return ring_areas
...
def plot(data_dict, fiducialize_num_start=0, fiducialize_num_end=16, area_normalize=True):
    ...
    for key, value in data_dict.items():
        ...
        if not area_normalize:
            plotting_data = value
        if pressure<750:
            plt.plot(ch,plotting_data, label=pressure)

        plotting_sets.append(plotting_data)
        i += 1
    plt.title('50V/cm')
    plt.ylabel('Counts per cm^2')
    plt.xlabel('Channel')
    plt.legend()
    plt.savefig('DataPlotMatchVargheseSimulation')

    standardDevs, mean = calculate_standard_deviations_mean(mm, plotting_sets)

    plot_sigma(pressures, standardDevs)
...
def calculate_standard_deviations_mean(mm, plotting_sets):
...
def plot_sigma(pressures, standardDevs):
    standardDevs = [standardDev/10 for standardDev in standardDevs]
    plt.figure()
...
def plot_diffusion_coefficient(pressures, Vd, standardDevs, filename):
    ...
    plt.title('Dt from Data Sigma/Mean at 500 & 50 V/cm')
