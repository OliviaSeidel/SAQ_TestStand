...
def plot(data_dict, fiducialize_num_start=0, fiducialize_num_end=16,DtUnits='cm^s/s', area_normalize=True, mean_from_data=False, fixed_mean = False):
    ...
    if mean_from_data:
        ...
        standardDevs, mean = calculate_standard_deviations_mean(mm, plotting_sets, meanVal = 'DataSet', DtUnits=DtUnits)
        plot_sigma(pressures, standardDevs, meanFrom = 'DataSet', DtUnits= DtUnits)
        plot_diffusion_coefficient(pressures, Vd, standardDevs, 'DtData', meanFrom='DataSet', DtUnits= DtUnits)
    if not mean_from_data:
        mean = fixed_mean
        standardDevs, mean = calculate_standard_deviations_mean(mm, plotting_sets, meanVal = mean, DtUnits=DtUnits)
        plot_sigma(pressures, standardDevs, meanFrom = mean, DtUnits = DtUnits)
        plot_diffusion_coefficient(pressures, Vd, standardDevs, 'DtData',meanFrom = mean, DtUnits = DtUnits)
...
def calculate_standard_deviations_mean(mm, plotting_sets, meanVal= 'DataSet', DtUnits='cm^s/s'):
    ...
    return standardDevs,mean

def plot_sigma(pressures, standardDevs, meanFrom= 'DataSet', DtUnits = 'DtUnits'):
    plt.figure()
...
def plot_diffusion_coefficient(pressures, Vd, standardDevs, filename, meanFrom = 'DataSet', DtUnits= 'DtUnits'):
    plt.figure()
    Dtvals = []
    ...
    if meanFrom == 'DataSet' :
        plt.title('Dt From Mean Calculated Per dataset at 500 & 50 V/cm')
    else:
        plt.title('Dt From Mean ' + str(meanFrom) + ' at 500 & 50 V/cm')
...
plot(data_dict,fiducialize_num_start=0, fiducialize_num_end=11, DtUnits='cm^s/s', area_normalize=True, mean_from_data=False, fixed_mean = 0.36 )
